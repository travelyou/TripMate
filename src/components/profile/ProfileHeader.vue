<script setup>
import { ref } from 'vue'
import { Camera, Tag, Settings, Pencil } from 'lucide-vue-next'
...
const emit = defineEmits(['edit-profile', 'edit-bio', 'update-avatar', 'open-friends'])
      ...
      <div class="md:hidden flex flex-col gap-4">
          ...
          <div class="flex-1 min-w-0 flex flex-col justify-center h-20">
            <div class="flex items-center justify-between mb-1">
              <div class="flex items-center gap-2 flex-wrap">
                <h1 class="text-xl font-bold tracking-tight text-white">{{ user.name || user.nickname || '用戶' }}</h1>
                <span class="px-2.5 py-1 bg-white/10 rounded-md text-xs font-medium text-white border border-white/20 flex items-center">
                  @{{ user.location || '台灣' }}
                </span>
              </div>
              <button
        ...
        <div class="text-sm">
           <div class="flex items-start gap-2 mb-2 relative">
             <p class="text-primary-100 font-light leading-relaxed line-clamp-2 flex-1">
            {{ user.bio || '這傢伙很懶，什麼都沒留下...' }}
          </p>
            <button
              v-if="isCurrentUser"
              class="p-1.5 bg-white/10 hover:bg-white/20 rounded-full text-white transition shrink-0"
              title="編輯自我介紹"
              @click="$emit('edit-bio')"
            >
          ...
          <div class="flex flex-wrap gap-1.5">
            <span
              ...
              :key="tag"
              class="px-1.5 py-0.5 bg-white/10 rounded-md text-[10px] font-medium text-white border border-white/20 flex items-center"
            >
      ...
      <div class="hidden md:flex items-end gap-6">
        ...
        <div class="flex-1 text-left">
          <div class="flex items-center justify-start gap-3 mb-2">
            <h1 class="text-3xl font-bold tracking-tight text-white">{{ user.name || user.nickname || '用戶' }}</h1>
            <span class="px-4 py-1.5 bg-white/10 rounded-lg text-sm font-medium text-white border border-white/20 flex items-center">
              @{{ user.location || '台灣' }}
            </span>
          ...
          <div class="flex items-start gap-2 mb-4 max-w-xl relative">
            <p class="text-primary-100 text-lg font-light leading-relaxed flex-1">
            {{ user.bio || '這傢伙很懶，什麼都沒留下...' }}
          </p>
            <button
              v-if="isCurrentUser"
          ...
          <div class="flex flex-wrap justify-start gap-2">
            <span
              v-for="tag in user.tags"
              :key="tag"
              class="px-2 py-1 bg-white/10 rounded-lg text-xs font-medium text-white border border-white/20 flex items-center"
            >
