...
const itemData = computed(() => ({
  ...
  image: props.traveler.banner_image,
  author: props.traveler.author_name,
  avatar: props.traveler.author_avatar,
  location: props.traveler.location,
  date: props.traveler.start_date,
  status: props.traveler.status,
  people: `${props.traveler.current_people}/${props.traveler.max_people}`,
  tags: props.traveler.tags,
  comments: props.traveler.comments_count,
}))
...
const dateRange = computed(() => {
  if (!props.traveler.start_date || !props.traveler.end_date) return ''
  return `${formatDate(props.traveler.start_date)} - ${formatDate(props.traveler.end_date)}`
})
</script>

<template>
  <div
    class="p-5 bg-[#fffef7] transition relative cursor-pointer border-4 border-amber-700 shadow-[4px_4px_0px_0px_rgba(139,111,71,0.2)] hover:shadow-[6px_6px_0px_0px_rgba(139,111,71,0.3)] hover:translate-y-[-2px]"
  >
    <div
    ...
    <div class="flex flex-col lg:flex-row gap-4">
      ...
      >
        <img
          :src="traveler.banner_image || 'https://picsum.photos/400/300'"
          :alt="traveler.title"
      ...
      <div class="flex-1 flex flex-col justify-between">
        <div class="flex items-center space-x-3 mb-2">
          <img
            :src="traveler.author_avatar"
            class="w-8 h-8 rounded-full object-cover border-2 border-gray-200"
          />
          <div>
            <div class="flex items-center space-x-1">
              <span class="font-bold text-sm text-gray-800">{{ traveler.author_name }}</span>
              <span
                class="text-xs font-semibold text-indigo-500 bg-indigo-50 px-1.5 py-0.5 rounded-full"
              >
                {{ traveler.spirit_animal }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-2 text-sm text-gray-700">
          ...
          <div class="flex items-center flex-wrap gap-4 mt-2">
            ...
            >
              ...
              />
              <span>{{ traveler.likes_count || 0 }}</span>
            </button>
          ...
          <div class="flex justify-between items-end pt-2 border-t border-gray-100">
            <div class="flex items-center text-gray-800 font-bold">
              ...
              <span class="text-lg text-blue-600 ml-1">
                {{ traveler.current_people }}/{{ traveler.max_people }}
              </span>
            </div>
