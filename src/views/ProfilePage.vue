...
// Mapping Itineraries to TravelerCard Structure for "Hosted Trips" tab
// TravelerCard expects: status, image, title, content, author, spiritAnimal, tags, location, date, comments, people
const hostedTrips = computed(() => {
  return itineraryStore.myItineraries.map((trip) => ({
    ...
    spiritAnimal: user.value.spiritAnimal || '🦁 樂天派', // Fallback if missing
    location: trip.location || '台灣', // Fallback as itinerary might not strictly have location field in generic summary
    date: trip.startDate,
    ...
    tags: ['行程', trip.status],
    isAuthor: true,
    // Mocking commentsData needed for modal? PostDetailModal uses commentsData
    commentsData: [],
  }))
})

// Filter for Posts
// PostCard/PostDetailModal expects: avatar, author, time, SPIRITANIMAL, title, content, image, tags, likes, comments, etc.
...
function addPlace(type) {
  const nameVal =
    type === 'domestic' ? newDomesticPlace.value.trim() : newInternationalPlace.value.trim()
  ...
  if (nameVal) {
    ...
    const newPlaceObj = {
      name: nameVal,
      date: dateVal || new Date().toISOString().slice(0, 7).replace('-', '.'),
    }
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 min-h-[400px] p-6">
          ...
          <div v-if="activeTab === 'hosted_trips'" class="space-y-4">
            <div v-if="hostedTrips.length > 0" class="grid grid-cols-1 gap-6">
              <TravelerCard
                v-for="trip in hostedTrips"
                :key="trip.id"
                :traveler="trip"
                @click="openPostDetailModal(trip, false)"
              />
            </div>
          ...
          <div v-if="activeTab === 'visited_places'" class="space-y-8 animate-fade-in">
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-3 mb-3">
                <div
                  v-for="(place, idx) in userStore.visitedPlaces.domestic"
                  :key="idx"
                ...
                >
                  ...
                  <button
                    ...
                    @click="removePlace('domestic', idx)"
                  >
                    <X class="w-4 h-4" />
                  </button>
                </div>
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-3 mb-3">
                <div
                  v-for="(place, idx) in userStore.visitedPlaces.international"
                  :key="idx"
                ...
                >
                  ...
                  <button
                    ...
                    @click="removePlace('international', idx)"
                  >
                    <X class="w-4 h-4" />
                  </button>
                </div>
              ...
              <div v-if="isCurrentUser" class="flex gap-2 max-w-md items-center">
                <input
                  ...
                  placeholder="新增國外城市..."
                  class="px-4 py-2 border rounded-xl text-sm flex-1 focus:ring-2 focus:ring-indigo-500 outline-none"
                  @keyup.enter="addPlace('international')"
                />
                <input
          ...
          <div v-if="activeTab === 'posts'" class="space-y-4">
            <div v-if="myPosts.length > 0">
              <PostCard
                ...
                @comment="openPostDetailModal(post, true)"
              />
            </div>
            <div v-else class="text-center py-20 text-gray-400">
              <p>還沒有發布過心得文章喔！</p>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 overflow-hidden">
          ...
          >
            <div
              v-if="userStore.wishlist.length === 0"
        ...
        <div class="p-6 space-y-6">
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2 mb-3">
              ...
              >
                {{ wish }}
                <button class="ml-1 hover:text-red-500" @click="editForm.wishlist.splice(idx, 1)">
                  <X class="w-3 h-3" />
                </button>
              </span>
            </div>
                ...
                @keyup.enter="
                  ($event) => {
                    if ($event.target.value.trim()) {
                      editForm.wishlist.push($event.target.value.trim())
                      $event.target.value = ''
                    }
                  }
                "
